...
```{r, load_spa, cache.lazy = TRUE, echo = TRUE, message = FALSE}
...
```{r, ret_mesh, cache.lazy = TRUE, echo = TRUE}
...
```{r, ret_genes, cache.lazy = TRUE, echo = TRUE}
library(RCurl)
library(XML)
...
```{r, est_drt, cache.lazy = TRUE, echo = TRUE, results = 'hide'}
...
```{r, tab_t2d_all, cache = TRUE, echo = FALSE}
library(kableExtra)
...
gtexv7_median_tpm <- CePa::read.gct(system.file("extdata", 'GTEx_Analysis_2016-01-15_v7_RNASeQCv1.1.8_gene_median_tpm.gct.gz', package="ThETA"))
...
efo.OBO <- get_OBO(system.file("extdata", 'efo_feb2019.obo', package="ThETA"), extract_tags = "everything") 
