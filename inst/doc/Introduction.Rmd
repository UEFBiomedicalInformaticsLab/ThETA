...
3. A tissue-specific efficacy (TSE) score is finnly estimated for all genes that are expressed in the tissues that are relevant for T2D.
...
The output is a *data.frame* object containing the TSE score for all pairs genes-tissue.
...
Instead, the following barplot reports the tissue-specific scores for the top 5 genes within each tissue.
...
* biological annotations which are significantly associated with a set of genes (by using th over-representation analysis);
