...
weighted.shortest.path <-
  function(disease_genes,
    ...
    if (is.null(names(dis_relevant_tissues)))
    ...
    if (length(sign_tiss) != 0) {
      if (verbose)
        print(paste(
          length(sign_tiss),
          ' tissue/s significant for given disease.',
          sep = ''
        ))
      ...
      new_sh.path <-
        mapply(function(x, y)
          t(x) * y[colnames(x)], sh.path, W[sign_tiss], SIMPLIFY = F)
      ...
      return(list(
        shortest_paths = target_path,
        shortest_paths_avg = target_path_mean
      ))
    }
    else
      stop('No tissue significant for given disease!')
  }
...
tissue.specific.scores <-
  function(disease_genes,
    ...
    if (is.null(disease_genes) |
        is.null(tissue_expr_data) |
        is.null(ppi_network) | is.null(dis_relevant_tissues))
      stop('Incomplete data input.')
    # compile the wsp
    wsp_list = weighted.shortest.path(
      disease_genes,
      ppi_network,
      directed_network,
      tissue_expr_data,
    ...
    dt <-
      data.frame(tissue_scores, avg_tissue_score = rowMeans(tissue_scores[, , drop =
                                                                            F]))
    return(dt)
  }
...
list.tissue.specific.scores <-
  function(disease_gene_list,
    ...
    require(foreach)
    require(doParallel)
    if (is.list(disease_gene_list) == FALSE)
    ...
    if (is.matrix(dis_relevant_tissues) == FALSE)
      stop('Argument dis_relevant_tissues is not a matrix!')
    if (is.null(rownames(dis_relevant_tissues)) |
        is.null(colnames(dis_relevant_tissues))) {
      stop('Both colnames and rownames for dis_relevant_tissues must be provided!')
    }
    common_diseases <-
      intersect(names(disease_gene_list), rownames(dis_relevant_tissues))
    if (length(common_diseases) == 0)
      stop('No disease in common between disease_gene_list and dis_relevant_tissues!')
    else if (length(common_diseases) != length(names(disease_gene_list))) {
      if (verbose)
        print(
          paste(
            length(common_diseases),
            'diseases in common between disease_gene_list and dis_relevant_tissues.',
            sep = ' '
          )
        )
    }
    ...
    if (!is.null(parallel)) {
      ...
      wsp <-
        sapply(common_diseases, function(i)
          weighted.shortest.path(
            disease_gene_list[[i]],
            ppi_network,
            directed_network,
    ...
    wsp <- do.call(function(...)
      mapply(list, ..., SIMPLIFY = F), wsp)
    tissue_scores <-
      lapply(wsp$shortest_paths, function(x)
    ...
    tissue_scores <-
      lapply(tissue_scores, function(x)
        data.frame(x, avg_tissue_score = rowMeans(x[, , drop = F])))
  ...
  function(tissue_scores,
    ...
    for (i in 1:length(sign_tiss)) {
      g <-
        igraph::graph_from_edgelist(as.matrix(ppi_network[, 1:2]), directed = T)
      igraph::E(g)$weight <-
        tissue_expr_data[ppi_network[, 1], sign_tiss[i]]
      # select disease genes active in this tissue and search for shortest paths
      tissue_disease_genes <-
        intersect(disease_genes, igraph::V(g)$name)
      shp_top_genes[[length(shp_top_genes) + 1]] <-
        lapply(top_targets, function(x) {
          ss = igraph::distances(
            g,
            v = x,
            to = tissue_disease_genes,
            mode =  "out",
            weights = NULL,
            algorithm = "dijkstra"
          )
          if (verbose)
            print(paste(
              ...
              x,
              " cannot reach ",
              length(which(is.infinite(ss))),
              " nodes",
              sep = ""
            ))
          suppressWarnings(selected_nodes <-
                             unique(unlist(
      ...
      rwr_top_genes[[length(rwr_top_genes) + 1]] <-
        as.numeric(
          dnet::dRWR(
            g,
            normalise = "none",
...
generate.ora.data <- function(input,
                              databases = c("GO", "KEGG"),
  ...
  for (i in 1:length(input)) {
    if ("GO" %in% databases) {
      list_annots[[length(list_annots) + 1]] <-
        clusterProfiler::enrichGO(
          unique(unlist(input[[i]])),
          orgdb_go,
          ont = "BP",
          pvalueCutoff = apval,
      ...
      annots <- c(annots, "GO")
      if (verbose)
        print(paste("ORA completed for GO-", names(input)[i], ".", sep = ""))
    }
    if ("KEGG" %in% databases) {
      ...
      if (verbose)
        print(paste("ORA completed for KEGG-", names(input)[i], ".", sep = ""))
    }
    if ("REACTOME" %in% databases) {
      list_annots[[length(list_annots) + 1]] <-
        ReactomePA::enrichPathway(unique(unlist(input[[i]])),
                                  pvalueCutoff = apval,
                                  readable = T)
      annots <- c(annots, "REACTOME")
      if (verbose)
        print(paste("ORA completed for REACTOME-", names(input)[i], ".", sep =
                      ""))
    }
  }
  ...
  function(ora_data,
    ...
    for (i in 1:length(ora_data)) {
      if ("dotplot" %in% set_plots) {
        list_plots[[length(list_plots) + 1]] <-
          enrichplot::dotplot(ora_data[[i]],
      ...
      if ("emapplot" %in% set_plots) {
        list_plots[[length(list_plots) + 1]] <-
          enrichplot::emapplot(ora_data[[i]], showCategory = showCategory) +
          ggplot2::theme(
            legend.title = ggplot2::element_text(size = font_size),
            legend.text = ggplot2::element_text(size = font_size)
          )
        plots <-
          c(plots, paste("emapplot", names(ora_data)[i], sep = "-"))
      }
      if ("cnetplot" %in% set_plots) {
        list_plots[[length(list_plots) + 1]] <-
          enrichplot::cnetplot(ora_data[[i]], showCategory = showCategory) +
          ggplot2::theme(
            legend.title = ggplot2::element_text(size = font_size),
            legend.text = ggplot2::element_text(size = font_size)
          )
        plots <-
          c(plots, paste("cnetplot", names(ora_data)[i], sep = "-"))
      }
      if ("upsetplot" %in% set_plots) {
        list_plots[[length(list_plots) + 1]] <-
          enrichplot::upsetplot(ora_data[[i]]) +
          ggplot2::theme(
            legend.title = ggplot2::element_text(size = font_size),
            legend.text = ggplot2::element_text(size = font_size)
          )
        plots <-
          c(plots, paste("upsetplot", names(ora_data)[i], sep = "-"))
      }
    }
    print(plots)
...
novelty.plots <-
  function(list_genes,
    ...
    print(pmc.genes)
    plot <-
      enrichplot::pmcplot(pmc.genes, pubmed[1]:pubmed[2]) + ggplot2::theme_minimal() +
      ggplot2::theme(
        legend.title = ggplot2::element_text(size = font_size),
      ...
      server = function(input, output, session) {
        ...
        output$network <- renderVisNetwork({
          ...
          node$title <-
            AnnotationDbi::mapIds(org.Hs.eg.db,
                                  ...
                                  verbose = FALSE)
          node$size <-
            rowMeans(tissue_scores[nb, input$tissue, drop = F]) * 20
        ...
        output$table <- renderTable({
          ...
          if (is.null(last_id) |
            ...
            validate(need(
              input$current_node_id %in% top_target,
